...
def is_in_stage(member: Any, target_channel_id: Optional[int]) -> bool:
    """True if the member is connected to the required stage/voice channel.
    No target configured means no restriction."""
    if not target_channel_id:
        return True
    voice = getattr(member, "voice", None)
    channel = getattr(voice, "channel", None) if voice else None
    return getattr(channel, "id", None) == target_channel_id
